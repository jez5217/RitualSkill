"""Trains a small bag-of-words softmax-regression sentiment classifier and
exports it to a real ONNX graph for the /think Classical Inference (0x0800)
demo. Replaces the previous hardcoded +1/-1 wordlist heuristic with actual
learned weights.

Usage:
    ml/.venv .. or repo-root .ml-venv .. python ml/train.py
"""
import json
import re
...
# Excluded so the model (and the word-highlighting it drives) learns from
# content words instead of picking up spurious correlations on function words
# in a dataset this small.
STOPWORDS = {
    "the", "a", "an", "and", "or", "but", "is", "was", "were", "are", "be",
    "been", "it", "its", "this", "that", "of", "to", "in", "on", "for", "with",
    "at", "as", "by", "from", "has", "had", "have", "i", "you", "we", "they",
}
...
def build_vocab(rows):
    counts = Counter()
    for r in rows:
        counts.update(set(tokenize(r["text"])))  # doc frequency, not raw term frequency
...
def export_onnx(W, b, vocab, out_path):
    ...
    gemm_node = helper.make_node(
        "Gemm", inputs=["input", "W", "b"], outputs=["logits"], alpha=1.0, beta=1.0, transB=0
    )
    ...
    model = helper.make_model(
        graph, producer_name="ritual-dapp-ml", opset_imports=[helper.make_opsetid("", 13)]
    )
    model.ir_version = 8
...
def main():
    train_rows = load_jsonl("ml/data/train.jsonl")
    val_rows = load_jsonl("ml/data/val.jsonl")
    ...
    out_dir = "frontend/public/models/sentiment"
    import os

    os.makedirs(out_dir, exist_ok=True)

    export_onnx(W, b, vocab, f"{out_dir}/model.onnx")

    # Per-word, per-class learned weights, exported alongside the model so the
    # frontend can highlight which words actually drove a prediction (the same
    # coefficients the Gemm node uses internally -- an exact explanation, not
    # an approximation).
    vocab_words_in_order = sorted(vocab, key=lambda w: vocab[w])
    weights_per_word = W.tolist()  # [vocab_size][n_classes]
    with open(f"{out_dir}/vocab.json", "w", encoding="utf-8") as f:
        json.dump(
            {"classes": CLASSES, "vocab": vocab_words_in_order, "weights": weights_per_word}, f
        )
