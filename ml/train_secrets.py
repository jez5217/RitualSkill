"""Trains the Secrets page's "should this be encrypted?" classifier -- same
approach as train.py/train_intent.py (numpy softmax regression over a
bag-of-words vocabulary, exported to ONNX), applied to a new task: flagging
text that looks like it discloses a secret/API key/PII (SECRET) versus safe
generic text (SAFE). Mirrors the real piiEnabled bool on the HTTP (0x0801)
and LLM (0x0802) precompiles.
"""
import json
import os
import re
...
def export_onnx(W, b, out_path):
    ...
    graph = helper.make_graph(
        [gemm_node, softmax_node], "secrets_linear", [input_tensor], [output_tensor], initializer=[w_init, b_init]
    )
...
def main():
    train_rows = load_jsonl("ml/data/secrets_train.jsonl")
    val_rows = load_jsonl("ml/data/secrets_val.jsonl")
    ...
    out_dir = "frontend/public/models/secrets"
