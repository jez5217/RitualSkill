...
export type Intent = "GREETING" | "OTHER" | "QUESTION" | "RITUAL";
...
async function loadVocab(): Promise<VocabMeta> {
  if (!vocabPromise) {
    vocabPromise = fetch("/models/intent/vocab.json").then((r) => {
      if (!r.ok) throw new Error(`Failed to load intent vocab.json: ${r.status}`);
      return r.json();
    });
  }
  return vocabPromise;
...
async function loadSession(): Promise<OrtNS.InferenceSession> {
  if (!sessionPromise) {
    sessionPromise = loadOrt().then((ort) =>
      ort.InferenceSession.create("/models/intent/model.onnx", { executionProviders: ["wasm"] })
    );
  }
...
export function preloadIntentModel(): void {
  void loadSession();
  void loadVocab();
}
...
export async function classifyIntent(text: string): Promise<IntentResult> {
  ...
  return { intent: vocab.classes[bestIdx], confidence: probs[bestIdx] };
}
